...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Potential solution: Network deployment }
    \begin{itemize}
    \item An increasing amount of base stations (BS) is used to cope with the increasing demand of high speed mobile networks\begin{itemize}
    \item Increasing amount of energy consumption
\item 70-80 $\%$ of energy is used by base stations 
    \end{itemize}
\item Energy aware Hertrogeneous network (Het-Net)\begin{itemize}
\item Mix of cell shapes and sizes
\item Macro and micro BS
\item Micro cells can offload macro cells $\rightarrow$ reducing energy consumption 
\item Macro cells are responsible for basic coverage
\item Micro cells provide higher data rates to nearby users
\end{itemize}
...
\end{frame}
\note{
Base stations are deployed because of the growth of the telecommunication industry
However, this hike in the deployment of base stations will lead to immense energy consumption, because in mobile networks 70–80 of the power is consumed by base stations (BSs).
This means an increase in CO2 emissions in the environment. In addition, energy bills represent a major chunk of the cost for wireless operators.
Therefore the development of ‘energy-aware Heterogeneous Networks (Het-Nets)’ are increasing. Het-Nets are a mix of various cell shapes and sizes. Including Macro and micro cells
By choosing the location of the microcells and relays optimally within the range of a macro cell, they can significantly offload the macrocell and produce energy savings while providing better coverage. Macro cells are responsible for the basic coverage of the cell users, and the small cells are effective in providing higher data rates to their nearby users in dense areas with reduced power consumption.

These optimizations are especially efficient in areas where extremely high capacity and data rates are needed like offices, shopping malls, subway stations, etc., 
where the user density is large. Since most of these places are indoor, indoor access points should be deployed so that energy wastage due to wall penetration can be avoided.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\begin{frame}
    ...
    \begin{center}
	\includegraphics[scale=0.7]{Illistration_of_the_number_of_active_macro_base_stations_vs_7_cell_LTE-advance}
	\end{center} 
\end{frame}
\note{
The number of active macro cells decrease when more micro cells are used allowing for the macro stations to go to sleep more often for more time, and thus decreasing the overall power usage
}
...
\note{
The more micro stations are used in the network the more power can be saved. This is due to the fact that the micro stations are being used for micro to micro communication bypassing the macro stations allowing the macro stations to sleep
}
%%%%%
\begin{frame}
...
\item Beam forming 
\item MIMO $\rightarrow$ Will boost peak data rates \begin{itemize}
...
\end{itemize}
    \end{itemize}
        \begin{center}
	\includegraphics[scale=0.6]{emil-5g-fig2}
	\end{center} 
...
Multiple-input multiple-output (MIMO) transmission which allows both multi-layer transmission for increased data rates, and beam forming to a certain extent.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Potential solution: Backhauling }
    \begin{itemize}
